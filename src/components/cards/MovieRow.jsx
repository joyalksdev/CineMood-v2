...
const MovieRow = ({ rowId, title, fetchFn }) => {
  ...
  // standard fetch effect with cleanup to prevent memory leaks on fast navigation
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    fetchFn().then((data) => {
  ...
  // calculates 80% of current view width for smooth carousel scrolling
  ...
  return (
    <section className="py-6 group/section relative overflow-hidden">
      {/* modal pops up only when a movie is clicked */}
      {selectedMovie && (
        <QuickViewModal movie={selectedMovie} onClose={() => setSelectedMovie(null)} />
      )}

      {/* header with branding and "Explore All" navigation */}
      ...
      <div className="relative group/carousel px-4 md:px-10">
        {/* desktop-only scroll buttons using glassmorphism styling */}
        ...
        <div ref={rowRef} data-lenis-prevent className={`${loading ? 'gap-3' : 'gap-0'} flex gap-1 md:gap-3 overflow-x-auto scrollbar-hide pb-6 snap-x snap-mandatory`}>
          {loading ? (
            ...
            movies.map((movie) => (
              ...
              >
                {/* poster with dynamic border glow on hover */}
                <div className="relative aspect-[2/3] rounded-xl overflow-hidden bg-[#0a0a0a] border border-white/5 transition-all duration-500 group-hover/card:border-green-400/30 shadow-2xl">
                  ...
                  {/* star rating badge fixed to the top right corner */}
                  ...
                  {/* mobile-specific watchlist icon (avoids hover triggers) */}
                  ...
                  {/* desktop hover content: title, info button, and watchlist toggle */}
                ...
                {/* visible metadata below the card for context */}
