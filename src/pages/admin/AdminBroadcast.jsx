...
import { Send, Users, User, RefreshCcw, LayoutList, Clock, ChevronRight } from "lucide-react";
...
const AdminBroadcast = () => {
  ...
  const handleUpdate = async (id, updatedData) => {
    try {
      await api.put(`/admin/notifications/${id}`, updatedData);
  ...
  const handleReset = () => {
    setFormData({ recipient: "", title: "", message: "", type: "info", scheduledAt: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      scheduledAt: isScheduled ? new Date(formData.scheduledAt).toISOString() : null 
    };

    try {
      await api.post("/admin/notifications", payload);
      toast.success(recipientId ? "Notification sent to user" : "Global broadcast sent");
      handleReset();
    } catch (err) {
    ...
    <div className="p-4 sm:p-6 md:p-10 bg-[#050505] min-h-screen text-white">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-8 md:mb-12">
        <div>
          <div className="flex items-center gap-3">
            ...
            <InfoTooltip 
              ...
              content="Send real-time alerts to every active user. Use 'Maintenance' for scheduled downtime, 'Urgent' for critical system updates, and 'General' for feature announcements. Once sent, these appear instantly in the user's notification feed."
            />
          </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          >
            <LayoutList size={14} /> <span className="hidden xs:inline">View History</span><span className="xs:hidden">History</span>
          </button>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
          ...
          <div className="bg-[#0A0A0A] border border-white/5 p-6 md:p-8 rounded-[2rem]">
            ...
            <div className="relative">
              ...
              >
                ...
                <option value="maintenance">Maintenance</option> {/* Fixed spelling */}
        ...
        <div className="lg:col-span-8 bg-[#0A0A0A] border border-white/5 p-6 md:p-10 rounded-[2.5rem] flex flex-col">
          <div className="space-y-8 flex-1">
            ...
            <div className="space-y-2">
              ...
              <textarea 
                ...
                className="w-full bg-transparent py-4 text-neutral-400 outline-none resize-none text-base md:text-lg leading-relaxed placeholder:text-neutral-800 custom-scrollbar"
              />
            </div>
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  {isScheduled && (
                    <input 
                      ...
                      className="flex-1 sm:flex-none bg-[#050505] border border-white/10 rounded-xl px-4 py-2.5 text-[10px] font-bold outline-none focus:border-[#FFC509] text-white"
                    />
                  )}
                  <button 
                    type="button"
                    onClick={() => setIsScheduled(!isScheduled)}
                    className={`whitespace-nowrap px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${isScheduled ? 'bg-red-500/10 text-red-500' : 'bg-white/10 text-white'}`}
                  >
          ...
          <button 
            disabled={loading}
            type="submit"
            className="mt-8 md:mt-12 w-full py-5 md:py-6 bg-[#FFC509] text-black rounded-2xl md:rounded-[1.5rem] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-[11px] md:text-[12px] flex items-center justify-center gap-3 hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-50 shadow-lg shadow-[#FFC509]/5"
          >
            {loading ? "SENDING..." : (
              <>
